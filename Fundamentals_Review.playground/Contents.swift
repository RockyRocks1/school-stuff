...
let courseName = "App Dev with Swift"
...
// Your answer: This fails because the constant courseName can not be modified
...
var isGamePuased = false
let maxPlayers = 40
// A constant can be used in place of a variable in physics calculations, such as the gravitational constant
...
if (age >= 13 && hasPermissionSlip) {
    print("Can go on the trip")
} else {
    print("Cannot go on the trip")
}
...
let quantity = 35
let sizeLabel = (quantity < 10) ? "Small" : (quantity < 50) ? "Medium" : "Large"
...
// 3 up to 30.
for multiple in stride(from: 3, through: 30, by: 3) {
...
print("First power of 2 greater than \(power)")
print("\n=== PART 8: Type Casting & Conversion (12 min) ===")
...
var quantityOrdered: Int = 3
var orderTotal = Double(quantityOrdered) * price
...
var ageInput: String = "15"
var ageAsInt: Int? = Int(ageInput)
print(ageAsInt ?? "invalid age")
...
// Your code here:
if let number = mysteryValue as? Int {
...
// A conversion from a variable may not contain an Int
...
for i in 1...7 {
    ...
    let passStatus: String = (score >= 70) ? "Pass" : "Fail"
    if passStatus == "Pass" {
        passingGradeCount += 1
    }
    print("Score: " + String(score) + " -> Grade: " + scoreGrade + " -> " + passStatus)
    
}
print("Number of passing grades (>= 70): \(passingGradeCount)")
